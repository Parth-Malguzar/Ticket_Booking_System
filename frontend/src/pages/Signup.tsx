...
const Signup = () => {
  ...
  const handleSubmit = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await api.post("/auth/signup", { name, email, password, rememberMe, isVendor });
      toast.success(res.data.message)
      if (res.data.pending) {
        setName("");
        setEmail("");
      ...
      // Regular users must verify email before login — don't auto-login here.
      setName("");
      setEmail("");
      setPassword("");
      setRememberMe(false);
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-950 p-8 shadow-2xl shadow-black/40">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 text-sm text-zinc-300 sm:grid-cols-2">
          ...
          {isVendor && (
            <p className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
              Vendor accounts are reviewed by an admin before login is allowed.
            </p>
          )}
